 ...
 * IP addr before connectessing.  Initializes internal variables.
 ...
 * @return - Success(SSHDriverSuccess) or
 * failure(SSHDriverErrorInvalidParameter).
 */
...
SSHDriverStatus SSHDriver::connectSSH() {
  ...
  // At this point the connection hasn't yet authenticated.  The first thing to
  // do is check the hostkey's fingerprint against the known hosts.
  ...
  if (auth_pw_ == 1) {
    ...
    if (libssh2_userauth_publickey_fromfile(session_, username_, rsapubbuff,
                                            rsabuff, password_)) {
 ...
 * @return - Success(SSHDriverSuccess) if it succeeds to write. The possible
 * error numbers are
 ...
 */
SSHDriverStatus SSHDriver::write(const char *buffer, size_t bufferSize,
                                 size_t *bytesWritten, int timeout) {
  ...
  *bytesWritten = 0;
  ...
  // Now we need to read back the same number of bytes, to remove the written
 ...
 * @return - Success or failure.
 */
SSHDriverStatus SSHDriver::read(char *buffer, size_t bufferSize,
                                size_t *bytesRead, int readTerm, int timeout) {
  ...
  *bytesRead = 0;
  ...
  while ((matched == 0) && (ctimesecs < time_at_timeout)) {
    rc = libssh2_channel_read(channel_, &buffer[*bytesRead],
...
double SSHDriver::SSHDriverCurrentTimeSecs() {
  ...
  if (!initialized) {
    if (QueryPerformanceFrequency(&proc_freq) != TRUE)
